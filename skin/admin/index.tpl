...
{block name='head:title'}{#brand#|ucfirst}{/block}
...
{block name='article:header'}
    ...
    <div class="pull-right hidden-ph hidden-xs">
        <p class="text-right">
            {#nbr_pages#|ucfirst}: {$pages|count}<a href="{$smarty.server.SCRIPT_NAME}?controller={$smarty.get.controller}&amp;action=add" title="{#add_brand#}" class="btn btn-link">
                <span class="fa fa-plus"></span> {#add_brand#|ucfirst}
            </a>
        </p>
    </div>
    {/if}
    <h1 class="h2"><a href="{$smarty.server.SCRIPT_NAME}?controller={$smarty.get.controller}" title="Afficher la liste des pages">{#brand#|ucfirst}</a></h1>
{/block}
    ...
    <div class="panels row">
        <section class="panel col-ph-12">
            ...
            <header class="panel-header panel-nav">
                <h2 class="panel-heading h5">{#root_brand#|ucfirst}</h2>
            ...
            <div class="panel-body panel-body-form">
                ...
                <div class="tab-content">
                    <div role="tabpanel" class="tab-pane {if !$smarty.get.plugin}active{/if}" id="list">
                        ...
                        {include file="section/form/table-form-3.tpl" data=$pages idcolumn='id_bd' activation=true sortable=$sortable controller="brand" change_offset=true}
                    </div>
                                            ...
                                            <div class="row">
                                                <div class="col-ph-12 col-sm-8">
                                                    <div class="form-group">
                                                        <label for="content[{$id}][brand_name]">{#brand_name#|ucfirst} :</label>
                                                        <input type="text" class="form-control" id="content[{$id}][brand_name]" name="content[{$id}][brand_name]" value="{$contentData.{$id}.name}" size="50" placeholder="{#brand_name_ph#|ucfirst}" />
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label for="content[{$id}][brand_content]">{#content#|ucfirst} :</label>
                                                <textarea name="content[{$id}][brand_content]" id="content[{$id}][brand_content]" class="form-control mceEditor">{call name=cleantextarea field=$contentData.{$id}.content}</textarea>
                                            </div>
                                            <div class="form-group">
