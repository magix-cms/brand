<div class="row">
    <form id="delete_img_brand" action="{$smarty.server.SCRIPT_NAME}?controller={$smarty.get.controller}&amp;action=delete" method="post" class="validate_form delete_form_img col-ph-12">
        <div class="form-group">
            <input type="hidden" id="del_img" name="del_img" value="{$page.id_bd}">
    ...
    {/foreach}
    <form id="edit_img_brand" action="{$smarty.server.SCRIPT_NAME}?controller={$smarty.get.controller}&amp;action=edit&amp;edit={$page.id_bd}" method="post" class="validate_form edit_form_img col-ph-12 col-md-6 col-lg-5">
        <h3>Image, description de l'image et SEO</h3>
        <div class="form-group">
            <label for="name_img_{$id}">{#name_img#|ucfirst} :</label>
            <input type="text" class="form-control" placeholder="{#ph_name_img#}" id="name_img_{$id}" name="name_img" value="{if isset($page.img_bd)}{$page.img_bd}{else}{$page.content[$default].url_bd}{/if}" />
        </div>
        <div id="drop-zone" class="img-drop{if !isset($page.imgSrc) || empty($page.imgSrc)} no-img{/if}">
            ...
            <div class="preview-img">
                <img id="preview"
                     src="{if isset($page.imgSrc) && !empty($page.imgSrc)}/upload/brand/{$page.id_bd}/{$page.imgSrc['original'].img}{else}#{/if}"
        ...
        <fieldset>
            ...
            {if $edit}
                <input type="hidden" name="brand[id]" value="{$page.id_bd}" />
            {/if}
